# -*- coding: utf-8 -*-
"""
Graph Attention Network (GATConv) for Node Classification - GRAPH-LEVEL (Inductive) SPLIT
...
Usage:
    python train_gat_from_pyg.py --input_dir C:/data/graphs_out --out_dir ./runs/gat_from_pyg
"""
...
try:
    ...
    from torch_geometric.nn import GATConv  # <--- 修改点 1：导入 GATConv
    PYG_AVAILABLE = True
except Exception:
    PYG_AVAILABLE = False
...
DEFAULT_OUT_DIR   = "./runs/gat_from_pyg" # <--- 修改点 (可选)：更新了默认输出目录
SEED              = 42
TRAIN_RATIO       = 0.7
VAL_RATIO         = 0.15
HIDDEN_DIM        = 64
...
def main():
    ...
    # 8) 模型
    # <--- 修改点 3：实例化 GAT2Layer
    model = GAT2Layer(in_dim, args.hidden_dim, num_classes,
    ...
    for epoch in range(1, args.epochs+1):
        ...
        tr_acc, tr_f1, _ = evaluate(model, train_loader, device, num_classes)
        va_acc, va_f1, _ = evaluate(model, val_loader, device, num_classes)
        te_acc, te_f1, _ = evaluate(model, test_loader, device, num_classes)
    ...
    for g_dict in all_graphs_dict:
        ...
        for i, nid in enumerate(data.node_ids):
            rows.append({
                "node_id": nid,
                "y_true": int(data.y[i].item()),
                "y_pred": int(pred[i]),
                "conf": float(probs[i, pred[i]])
            })
        out_csv = out_dir / f"pred_{data.key}.csv"
        with out_csv.open("w", newline="", encoding="utf-8") as f:
            w = csv.DictWriter(f, fieldnames=["node_id","y_true","y_pred","conf"])
            w.writeheader()
            w.writerows(rows)

    print(f"Done. Best val F1={best_val_f1:.3f} (epoch {best_epoch}). Artifacts saved in: {out_dir}")
